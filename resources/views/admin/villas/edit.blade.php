        ...
        <div style="background: #f8f9fa; padding: 20px; min-height: 100vh;">
            ...
            <nav style="display: flex; flex-direction: column; gap: 10px;">
                <a href="{{ route('admin.dashboard') }}"
                    style="padding: 12px; background: white; color: #f8f9fa; text-decoration: none; border-radius: 4px;">Dashboard</a>
            ...
            <form action="{{ route('admin.villas.update', $villa) }}" method="POST" enctype="multipart/form-data"
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <select name="status" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;"
                        ...
                        <option value="active" {{ old('status', $villa->status) === 'active' ? 'selected' : '' }}>Active
                        </option>
                        <option value="inactive" {{ old('status', $villa->status) === 'inactive' ? 'selected' : '' }}>Inactive
                        </option>
                        <option value="maintenance" {{ old('status', $villa->status) === 'maintenance' ? 'selected' : '' }}>
                            Maintenance</option>
                        <option value="available" {{ old('status', $villa->status) === 'available' ? 'selected' : '' }}>
                            Available (Legacy)</option>
                        <option value="unavailable" {{ old('status', $villa->status) === 'unavailable' ? 'selected' : '' }}>
                            Unavailable (Legacy)</option>
                    </select>
